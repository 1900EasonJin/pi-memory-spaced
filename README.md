...
### 间隔重复
每条记忆有 **效力分数（potency）**，随时间指数衰减。低分自动归档，不再注入。

```
potency 变化曲线：
  新记忆 → 0.8 ──→ 每天 ×0.95 ──→ 低于 0.2 归档
             ↑
             └── 提取器发现新独立证据时 +0.01（evidenceCount+1）
```

**强化只来自新证据，注入/读取不强化**：被注入只累计 `accessCount`（可能固化为永久记忆），不改 potency、不重置衰减锚点——被频繁使用的记忆仍会随时间衰减。

### 自动提取
每个 session 在用户消息 ≥3 轮后触发一次（`agent_settled`），当前会话模型分析**整个会话**的用户/助手消息，提取需要长期记住的信息。工具输出不会发送给提取模型。

**硬闸门（不依赖模型自觉）**：LLM 每条候选必须显式声明 `kind` 与 `durable`——
- `kind`：`preference`（领导偏好）/ `workflow`（工作方式）/ `constraint`（约束约定）/ `lesson`（踩坑教训）/ `decision`（架构决策）/ `project_fact`（一次性项目细节）
- `durable !== true` 或 `kind === "project_fact"` → 解析阶段直接丢弃

拒绝入库：当前分支/提交状态、文件路径、构建产物、单次报错与修复、临时进度、可从仓库重新读取的事实、随时间失效的快照。单次行为不推断为偏好，除非用户明确表达（如「以后都这样做」）。

### 安全去重
新记忆计算字符重叠相似度，自动处理遵循保守规则：
- **匹配（similarity ≥ 0.45，含 exact/high/mid）→ 只强化不新增**：`evidenceCount+1`、potency +0.01、paths/tags 并集；内容保持旧条目原文（高相似可能是纠正或否定，不冒险覆写）
- **未匹配（< 0.45）→ 才允许新增**（受配额限制）
- 用户手动添加的相似内容 → 保留，因为它可能是纠正、否定或新约定

### 自动整合
相似度在「相关但不同」区间（0.3~0.8）的记忆会在后台定期整合，避免同一主题裂成多条：

- **触发**：每轮对话结束后检查——本轮有新增且活跃记忆 >30 条，或距上次整合超过 24 小时
- **聚类**：本地 2-gram 相似度 ≥0.3 的并查集连簇，无簇则不消耗 LLM 调用
- **合并**：每个簇由当前会话模型合成一条更完整的记忆（paths/tags/accessCount 并集，potency 取最大值）
- **安全**：用户口述「记住：」的记忆和已固化记忆不参与自动合并；LLM 可否决不该合的簇；落库前自动备份 `memory-store.backup.json`
...
- `≥0.30` → 活跃
- `0.20~0.30` → 低效记忆
- `<0.20` → 已归档（保留在 Store，但不自动注入）

**容量控制（防大爆炸）**：
- 每个 session 只自动提取一次（用户消息 ≥3 轮后触发）
- 每日自动新增上限 10 条，库存总量上限 300 条，超限后只强化已有记忆
- 提取器对相似记忆的强化增量 +0.01（evidenceCount+1），注入不产生任何强化
